// Vendored verbatim from topjohnwu/zygisk-module-sample @ 7bb941a (module/jni/zygisk.hpp).
//
// Pinned to Zygisk API **v4** on purpose. Upstream HEAD is v5, and a v5 build of this module is
// silently rejected by the Magisk on the target device (30.7): the .so loads but Zygisk never
// calls into it, so postAppSpecialize never runs and the module logs nothing at all. Verified by
// building both and watching `logcat -s ExtDensityZ:V` after a force-stop + relaunch of an
// allowlisted app - v4 logs and hooks, v5 is silent. Raise this only alongside a Magisk that
// advertises the newer API.
